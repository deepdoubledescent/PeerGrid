...
import { getEvent, updateEvent, getEventTopics, getLocations } from "./Controller";
...
export default function EditEventPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [evt, topicRows] = await Promise.all([
          getEvent(eventId),
          getEventTopics()
        ]);
        ...
        if (!evt.can_edit) {
          setStatus({ type: "error", message: "You do not have permission to edit this event." });
          setLoading(false);
          return;
        }

        const normalizedTopics = (topicRows || [])
          .map((t) => normalizeTopicLabel(typeof t === "string" ? t : t?.topic_name))
          .filter(Boolean)
          .sort((a, b) => a.localeCompare(b));

        const existingEventTopics = (evt.topics || [])
          .map((topic) => normalizeTopicLabel(topic))
          .filter(Boolean);

        setEvent(evt);
        setTopics(
          Array.from(new Set([...normalizedTopics, ...existingEventTopics])).sort((a, b) =>
            a.localeCompare(b)
          )
        );
        ...
        setPlace(evt.place || "");
        setIsOnline(!!evt.is_online);
        setEventDate(
          evt.event_date
            ? new Date(
                new Date(evt.event_date).getTime() -
                  new Date(evt.event_date).getTimezoneOffset() * 60000
              )
                .toISOString()
                .slice(0, 16)
            : ""
        );
        setSelectedTopics(existingEventTopics);
        setAllowApplicationCountVisible(!!evt.allow_application_count_visible);
        setAllowApplicantsVisible(!!evt.allow_applicants_visible);
      } catch (err) {
        console.error(err);
        setStatus({ type: "error", message: "Failed to load event." });
      } finally {
        setLoading(false);
      }
    };
  ...
  const validate = () => {
    ...
    if (!isOnline && !location.trim()) return "Location is required unless online.";
    return null;
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      navigate(`/events/${updated.id || eventId}`);
    } catch (err) {
      console.error(err);
      setStatus({
        type: "error",
    ...
    return (
      <div className="p-8 max-w-4xl mx-auto">
        <div className="text-red-600">
          {status.message || "You do not have permission to edit this event."}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-semibold mb-2">Edit Event</h1>
