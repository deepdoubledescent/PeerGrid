...
import { getLikedPeopleForUser, toggleLikePerson, getLocations, getInstitutes } from "./Controller";
...
export default function FollowingPage({ user }) {
  ...
  const fetchFollowing = async () => {
    ...
    try {
      const result = await getLikedPeopleForUser(user.id);
      setPeople(result || []);
    } catch (err) {
      console.error(err);
      setStatus({
  ...
  const onToggleLike = async (likedUserSub) => {
    try {
      ...
      setStatus({
        type: "error",
        message: "Could not update following status.",
      });
    }
  };
  ...
  }, [institution, showInstituteSuggestions]);
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <h1 className="text-3xl font-semibold mb-2">Following</h1>
      ...
      {loading ? (
        ...
        <div className="text-center text-stone-500 mt-10">
          {people.length === 0 ? "You are not following anyone yet." : "No people found."}
        </div>
      ) : (
